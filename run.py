...
with open('/etc/Covid19_web_config.json') as config_file:
...
def home():
    historical_data, _ = get_historical_data(path_historical, path_log_historical)
...
def country(country_code):
    ...
    historical_data['deathsMovingAverage'] = historical_data['deaths'].rolling(window=7).mean()
    historical_data['casesMovingAverage'] = historical_data['cases'].rolling(window=7).mean()
    historical_data['deathsAccum'] = historical_data['deaths'].cumsum()
    historical_data['casesAccum'] = historical_data['cases'].cumsum()
    historical_data['deathsChange'] = historical_data['deathsAccum'].pct_change() * 100
    historical_data['casesChange'] = historical_data['casesAccum'].pct_change() * 100
